        ...
        <div id="content">
            ...
            <div class="msgWrap bookclub"> <!-- 2022-03-07 변경: 학생별 클래스명 추가 초:ele / 중:mid / 고:high / 북클럽:bookclub-->
                ...
                <div class="msgBox">
                    <!--쪽지탭//-->
                    <ul class="msgTab">
                        <li class="on"><a href="./receiveList.do" target="_self">받은 쪽지함</a></li>
                        <li><a href="./sendList.do" target="_self">보낸 쪽지함</a></li>
                    </ul>
                    <!--//쪽지탭-->
                    <!--쪽지 리스트//-->
                    <ul class="msgList">
                    	<li class="status">
                            ...
                            <button class="btn">삭제</button><button class="btn">읽음</button>
                            <p>
                            	<c:if test="${not empty maps.total_count}">
                            		<span>${maps.total_count}개</span>
                            	</c:if>
                            	<c:if test="${empty maps.total_count}">
									<span>0개</span>
                            	</c:if>
                            </p>
                        </li>
                    	<c:if test="${not empty list}">
	                    	<c:forEach var="list" items="${list}">
	                    		<c:if test="${list.readState eq 'Y' and not empty list.orgFileName}">
			                        <li>
			                            <input type="checkbox" class="cc" id="${list.messageIdx}" name="" />
			                            <label for="${list.messageIdx}"><span></span>선택</label>
			                            <a href="./receiveDetail.do?messageIdx=${list.messageIdx}" target="_self">
			                                <p class="name">${list.senderName}</p>
			                                <span class="title">${list.messageTitle}</span><u class="file">첨부파일</u>
			                                <span class="date">${list.sendDate}</span>
			                            </a>
			                        </li>
		                        </c:if>
		                        <c:if test="${list.readState eq 'Y' and empty list.orgFileName}">
			                        <li>
			                            <input type="checkbox" class="cc" id="${list.messageIdx}" name="" />
			                            <label for="${list.messageIdx}"><span></span>선택</label>
			                            <a href="./receiveDetail.do?messageIdx=${list.messageIdx}" target="_self">
		                        ...
		                        <c:if test="${list.readState eq 'N' and not empty list.orgFileName}">
			                        <li class="noRead">
			                            <input type="checkbox" class="cc" id="${list.messageIdx}" name="" />
			                            <label for="${list.messageIdx}"><span></span>선택</label>
			                            <a href="./receiveDetail.do?messageIdx=${list.messageIdx}" target="_self">
		                        ...
		                        <c:if test="${list.readState eq 'N' and empty list.orgFileName}">
			                        <li class="noRead">
			                            <input type="checkbox" class="cc" id="${list.messageIdx}" name="" />
			                            <label for="${list.messageIdx}"><span></span>선택</label>
			                            <a href="./receiveDetail.do?messageIdx=${list.messageIdx}" target="_self">
                        ...
                        <c:if test="${empty list}">
                        	<li class="noRead">
	                                <span class="title">${noReceiveList}</span>
	                            </a>
	                        </li>
                        </c:if>
                    </ul>
