	...
	<div id="wrap" class="wrap">
		...
		<div id="content" class="pb_0">
			...
			<div class="oneBoardWrap ">
				<!--탭//-->
				<ul class="boardTab">
					<li class="on"><a href="oneboard.html" target="_self">1:1 문의글 작성</a></li>
					<li><a href="oneboard_01.html" target="_self">내가 작성한 글</a></li>
				</ul>
				<!--//탭-->
				<!--문의 작성//-->
				<form class="boardBox" action="./modifyBoard.do?boardIdx=${boardView.boardIdx}" method="post">
					<em	class="notice">필수 입력항목</em>
					<ul class="boardWrite">
						<li class="selList">
							<select id="ss1" name="ss1" onchange="optionChange(this)">
								<option value="" disabled="disabled" <c:if test="${boardView.division1 eq ''}">selected</c:if>>선택</option>
								<option value="밀크T초등" <c:if test="${boardView.division1 eq '밀크T초등'}">selected</c:if>>밀크T초등</option>
								<option value="밀크T중등" <c:if test="${boardView.division1 eq '밀크T중등'}">selected</c:if>>밀크T중등</option>
								<option value="T CLASS" <c:if test="${boardView.division1 eq 'T CLASS'}">selected</c:if>>T CLASS</option>
							</select>
							<select id="ss2" name="ss2">
								<c:choose>
									<c:when test="${boardView.division1 eq '밀크T초등' or boardView.division1 eq '밀크T중등'}">
									<option value="" disabled="disabled">선택</option>
										<option value="학습/평가" <c:if test="${boardView.division2 eq '학습/평가'}">selected</c:if>>학습/평가</option>
										<option value="결제" <c:if test="${boardView.division2 eq '결제'}">selected</c:if>>결제</option>
										<option value="회원정보" <c:if test="${boardView.division2 eq '회원정보'}">selected</c:if>>회원정보</option>
										<option value="탭장애" <c:if test="${boardView.division2 eq '탭장애'}">selected</c:if>>탭장애</option>
										<option value="기타" <c:if test="${boardView.division2 eq '기타'}">selected</c:if>>기타</option>
									</c:when>
									<c:otherwise>
										<option value="" disabled="disabled">선택</option>
										<option value="이용문의" <c:if test="${boardView.division2 eq '이용문의'}">selected</c:if>>이용문의</option>
										<option value="건의/개선" <c:if test="${boardView.division2 eq '건의/개선'}">selected</c:if>>건의/개선</option>
										<option value="기타" <c:if test="${boardView.division2 eq '기타'}">selected</c:if>>기타</option>
									</c:otherwise>
								</c:choose>
							</select>
						...
						<li>
							<input type="text" id="boardTitle" name="boardTitle" placeholder="제목" value="${boardView.boardTitle}" />
						</li>
						<script >
						...
						<li class="itxtArea">
							<textarea id="textzone" class="itxtArea" name="boardContent" placeholder="내용">${boardView.boardContent}</textarea>
						</li>
					</ul>

					<!-- <button class="button blue big mt_20" onclick="location.href='oneboard_01.html' ">등록</button> -->
					<ul class="btnList mt_30"><!-- 수정일때 -->
						<li><button type="reset" class="button gray big">취소</button></li>
						<li><button id="btn_submit" class="button blue big">등록</button></li>
					</ul>
				</form>
				<!--//문의 작성-->
				 <!--안내//-->
