...
public class ReceiveListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(session.getAttribute("memberId") != null) {
			...
			maps.put("receiverName", session.getAttribute("name"));
			
			MessageDAO dao = new MessageDAO();
			List<MessageDTO> list = dao.getReceiveList(maps);
			total_count = dao.getReceiveListCount(maps);
			dao.close();
			...
			String pagingArea = BbsPage.pagingArea(total_page, page_no, page_block_start, page_block_end, "./receiveList.do?");
			...
			if(list != null && total_count != 0) {
				...
				req.setAttribute("noReceiveList", "받은 쪽지가 없습니다.");
